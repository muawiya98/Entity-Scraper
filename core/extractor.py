...
def _is_probably_name(text: str) -> bool:
    ...
    if not (1 <= len(words) <= 5):
        return False
    # Arabic names: accept if it contains Arabic letters.
    if _ARABIC_RE.search(text):
        return True
    # Latin names: most words should start uppercase.
    cap = sum(1 for w in words if w[:1].isupper())
    return cap >= max(1, len(words) - 1)


# def _json_ld_blocks(soup: BeautifulSoup) -> list:
#     blocks = []
...
def extract_description(soup: BeautifulSoup) -> str:
    ...
    # الاستفادة من remove_tags لتنظيف النصوص من أي شوائب برمجية
...
def _line_has_role(line: str) -> str | None:
    ...
    for kw in ROLE_KEYWORDS:
        if kw in low or kw in line:  # Arabic keywords are case-identical
            return kw
    return None
...
def _people_from_cards(soup: BeautifulSoup) -> list[dict]:
    ...
    # Candidate containers commonly used for team members.
    containers = soup.select(
        "div, li, article, section, td, figure"
    )
    for c in containers:
        # Skip huge containers (whole page sections).
        text = c.get_text("\n", strip=True)
        ...
        # Find a role line that is NOT the name itself.
        position = ""
        ...
        # Optional per-person contact details inside the card.
        email = ""
...
def _people_from_text(text: str) -> list[dict]:
    ...
    for raw in text.split("\n"):
        ...
        if not role:
            continue
        # Split on common separators between name and title.
        for sep in (" - ", " – ", " — ", ", ", " | ", ": ", " / "):
            if sep in line:
                left, right = line.split(sep, 1)
                left, right = left.strip(), right.strip()
                # Decide which side is the name vs the title.
...
def extract_people(soup: BeautifulSoup, text: str, json_ld: list) -> list[dict]:
    ...
    # De-duplicate by lower-cased name, keeping the richest record.
...
def parse_page(html: str, url: str, region: str = "SA") -> dict:
    ...
    # Drop noise that pollutes text extraction.
    for tag in soup(["script", "style", "noscript", "svg"]):
        tag.decompose()
    text = soup.get_text("\n", strip=True)
    # json_ld = _json_ld_blocks(BeautifulSoup(html, "lxml"))  # re-parse to keep scripts
