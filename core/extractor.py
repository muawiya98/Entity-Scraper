...
def extract_address(
    soup: BeautifulSoup, text: str, json_ld: list
) -> tuple[str, str, str]:
        ...
        for name, domains in SOCIAL_PATTERNS.items():
            ...
            if any(d in low for d in domains):
                ...
                if (
                    "sharer" in low
                    or "intent" in low
                    or low.rstrip("/").endswith(tuple(domains))
                ):
                    continue
                social[name] = href
    return social
...
def _people_from_cards(soup: BeautifulSoup) -> list[dict]:
    ...
    containers = soup.select("div, li, article, section, td, figure")
    for c in containers:
...
def _merge_llm_extract(result: dict, llm_data: dict) -> dict:
    ...
    if address and not current_addr:
        ...
        current_addr and (city or country) and (not current_city or not current_country)
    ):
        result["address"] = (
            current_addr,
